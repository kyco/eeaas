...
eeaas.register({
  ...
  onStart() {
    ...
    const wrapperId = `nyancat-wrapper-${Date.now()}`
    const trail: number[] = []
    let mouseX = 100
    let mouseY = 100
    let posX = 0
    ...
    let nyanEl: HTMLElement | null = null
    let rainbowEls: NodeListOf<HTMLElement> = [] as any
  ...
  onStop() {
    ...
    const data = (wrapper as any).__nyancatData
    if (data) {
      document.removeEventListener('mousemove', data.onMouseMove)
      data.intervalIds.forEach(clearInterval)
    }
